...
export default function Page() {
  ...
  let searchRef: { focus: () => void } | undefined
  ...
  // Refresh file tree when AI session produces file changes (Requirements 8.2, 8.3)
  createEffect(
    on(
      () => diffs().length,
      (len, prev) => {
      ...
      <div class="flex-1 min-h-0 flex flex-col md:flex-row">
        ...
        <div
          ...
          style={{
            ...
            "min-width": desktopSidePanelOpen() ? "400px" : undefined,
        ...
        >
          ...
          <Show when={desktopSidePanelOpen()}>
            <ResizeHandle
              ...
              min={400}
              max={typeof window === "undefined" ? 1000 : window.innerWidth * 0.6}
        ...
        <SessionSidePanel
          ...
          onOpenInTerminal={(dir) => {
            ...
            terminal.new({ cwd: dir })
          }}
        />

        <Show when={isDesktop() && ui.searchOpen}>
          <aside class="relative shrink-0 h-full border-l border-border-weak-base w-[360px] min-w-[280px] bg-background-stronger">
            <SearchPanel
              ref={(api) => (searchRef = api)}
              onClose={() => setUi("searchOpen", false)}
              onOpenFile={(path, line) => {
                file.load(path)
                tabs().open(file.tab(path))
                if (!view().reviewPanel.opened()) view().reviewPanel.open()
              }}
            />
          </aside>
