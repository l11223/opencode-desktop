...
export const SessionItem = (props: SessionItemProps): JSX.Element => {
  ...
  const ago = createMemo(() => relativeTime(props.session.time.updated ?? props.session.time.created, now()))
  ...
  const isArchived = createMemo(() => !!props.session.time?.archived)
      ...
      <div class="flex items-center gap-1 w-full">
        ...
        >
          <Switch fallback={<Icon name="dash" size="small" class="text-icon-weak" />}>
            <Match when={isArchived()}>
              <Icon name="archive" size="small" class="text-icon-weaker" />
            </Match>
        ...
        <Show when={!props.session.summary}>
          <span class="shrink-0 text-12-regular text-text-weak group-hover/session:hidden group-active/session:hidden group-focus-within/session:hidden">
            {ago()}
          </span>
        </Show>
        <Show when={props.session.summary}>
  ...
  return (
    <ContextMenu modal={!props.sidebarHovering()}>
      <ContextMenu.Trigger
        ...
        classList={{ "opacity-70": isArchived() }}
      >
        <Show
          when={hoverEnabled()}
          fallback={
      ...
      <ContextMenu.Portal mount={!props.mobile ? props.nav() : undefined}>
        <ContextMenu.Content>
          <ContextMenu.Item onSelect={() => props.onRename?.(props.session)}>
          ...
          <ContextMenu.Item onSelect={() => props.onDelete?.(props.session)}>
