...
import { childMapByParent, filterSessions, sortedRootSessions, archivedRootSessions } from "./helpers"
...
export const SortableWorkspace = (props: {
  ...
  const archived = createMemo(() => archivedRootSessions(workspaceStore))
  const filtered = createMemo(() => filterSessions(query(), sessions(), query() ? archived() : undefined))
        ...
        <Collapsible.Content>
          <nav class="flex flex-col gap-1 px-2">
            <Show when={sessions().length > 0}>
              <div class="pb-1">
                <div class="flex items-center gap-1.5 px-2 h-8 rounded-md border border-border-weak-base bg-input-base focus-within:border-transparent focus-within:shadow-[0_0_0_3px_var(--border-weak-selected),0_0_0_1px_var(--border-selected)]">
                  <Icon name="magnifying-glass" size="small" class="text-icon-weak shrink-0" />
                  <input
                    type="text"
                    placeholder={language.t("session.search.placeholder") ?? "Search sessions..."}
                    value={query()}
                    onInput={(e) => setQuery(e.currentTarget.value)}
                    class="flex-1 min-w-0 bg-transparent border-none outline-none text-14-regular text-text-strong placeholder:text-text-weak"
                  />
                  <Show when={query()}>
                    <IconButton
                      icon="close"
...
export const LocalWorkspace = (props: {
  ...
  const [query, setQuery] = createSignal("")
  const archived = createMemo(() => archivedRootSessions(workspace().store))
  const filtered = createMemo(() => filterSessions(query(), sessions(), query() ? archived() : undefined))
