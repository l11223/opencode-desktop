                ...
                <For each={props.terminal.all()}>
                  {(pty) => {
                    const layout = createMemo(() => {
                      if (props.terminal.active() !== pty.id) return undefined
                      return props.terminal.getLayout()
                    })
                    return (
                      <div
                        ...
                        >
                          {(l) => (
                            <SplitContainer
                              ...
                              onFocus={(id) => props.terminal.focusPane(id)}
                              onResize={(path, ratio) => props.terminal.resizePane(path, ratio)}
                              renderPane={(id) => {
                                const p = createMemo(() => props.terminal.all().find((t: LocalPTY) => t.id === id))
                                return (
                                  <Show when={p()}>
                                    {(found) => (
                                      <Terminal
